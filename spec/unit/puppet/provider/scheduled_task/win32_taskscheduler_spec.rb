...
describe Puppet::Type.type(:scheduled_task).provider(:taskscheduler_api2) do
  subject(:taskscheduler_api2) { described_class.new }

  before :each do
    skip('Not on Windows platform') unless Puppet.features.microsoft_windows?
  end

  it 'is the default provider' do
    expect(Puppet::Type.type(:scheduled_task).defaultprovider).to eq(taskscheduler_api2.class)
  end
end
...
task_providers.each do |task_provider|
  ...
  describe Puppet::Type.type(:scheduled_task).provider(task_provider) do
    ...
    describe 'when retrieving' do
      ...
      it 'gets the working directory from the working_directory on the task' do
      ...
      it 'gets the command from the application_name on the task' do
        mock_task.expects(:application_name).returns('C:\Windows\System32\notepad.exe')

        expect(resource.provider.command).to eq('C:\Windows\System32\notepad.exe')
      end

      it 'gets the command arguments from the parameters on the task' do
      ...
      it 'gets the compatibility from the parameters on the task' do
      ...
      it 'gets the user from the account_information on the task' do
      ...
      describe 'whether the task is enabled' do
        ...
        it 'does not consider triggers for determining if the task is enabled' do
          mock_task.stubs(:enabled).returns(:true)
    ...
    describe '.instances' do
      it 'uses the list of task names to construct the list of scheduled_tasks' do
    ...
    describe '#user_insync?' do
      ...
      it 'considers the user as in sync if the name matches' do
        Puppet::Util::Windows::SID.expects(:name_to_sid).with('joe').twice.returns('SID A')

        expect(resource).to be_user_insync('joe', ['joe'])
      end

      it 'considers the user as in sync if the current user is fully qualified' do
      ...
      it 'considers a current user of the empty string to be the same as the system user' do
        Puppet::Util::Windows::SID.expects(:name_to_sid).with('system').twice.returns('SYSTEM SID')

        expect(resource).to be_user_insync('', ['system'])
      end

      it 'considers different users as being different' do
        Puppet::Util::Windows::SID.expects(:name_to_sid).with('joe').returns('SID A')
    ...
    describe '#trigger_insync?' do
      ...
      it 'does not consider any extra current triggers as in sync' do
        current = [
      ...
      it 'does not consider any extra desired triggers as in sync' do
      ...
      it 'considers triggers to be in sync if the sets of current and desired triggers are equal' do
        current = [
          { 'start_date' => '2011-9-12', 'start_time' => '15:15', 'schedule' => 'once' },
          { 'start_date' => '2012-10-13', 'start_time' => '16:16', 'schedule' => 'once' },
        ]
    ...
    describe '#triggers_same?' do
      ...
      it 'does not mutate triggers' do
      ...
      it "does not consider a disabled 'current' trigger to be the same" do
      ...
      it 'does not consider triggers with different schedules to be the same' do
      ...
      it 'does not consider triggers with different monthly types to be the same' do
      ...
      describe 'comparing daily triggers' do
        it "considers 'desired' triggers not specifying 'every' to have the same value as the 'current' trigger" do
          current = { 'schedule' => 'daily', 'start_date' => '2011-9-12', 'start_time' => '15:30', 'every' => 3 }
          desired = { 'schedule' => 'daily', 'start_date' => '2011-09-12', 'start_time' => '15:30' }
        ...
        it 'does not consider differences in date formatting to be different triggers' do
        ...
        it 'does not consider differences in time formatting to be different triggers' do
        ...
        it "considers different 'every' as different triggers" do
          current = { 'schedule' => 'daily', 'start_date' => '2011-9-12', 'start_time' => '15:30', 'every' => 3 }
      ...
      describe 'comparing monthly date-based triggers' do
        ...
        it "considers different 'on' as different triggers" do
          current = { 'schedule' => 'monthly', 'start_date' => '2011-9-12', 'start_time' => '15:30', 'months' => [1, 2], 'on' => [1, 3, 5, 7] }
      ...
      describe 'comparing weekly triggers' do
        it "considers 'desired' triggers not specifying 'day_of_week' to have the same value as the 'current' trigger" do
    ...
    describe '#validate_trigger' do
      ...
      it 'succeeds if all passed triggers translate from hashes to triggers' do
      ...
      it 'uses the exception from from_manifest_hash when it fails' do
        ...
        expect { provider.validate_trigger(triggers_to_validate) }.to raise_error(
          %r{#{Regexp.escape("Unknown trigger option(s): ['this is invalid']")}},
        )
      end

      it 'raises when an invalid day_of_week is passed' do
        triggers_to_validate = [
    ...
    describe '#validate_name' do
      context 'when the compatibility is 1' do
        ...
        it 'raises an error if the compatibility is less than 2 or the provider is win32_taskscheduler' do
          case task_provider
          when :win32_taskscheduler
            expect { resource.validate }.to raise_error(
              Puppet::ResourceError,
              %r{Use the taskscheduler_api2 provider instead.},
            )
          when :taskscheduler_api2
            expect { resource.validate }.to raise_error(
              Puppet::ResourceError,
              %r{Specify a compatibility of 2 or higher or do not specify a subfolder path.},
            )
          end
        end
      end
      ...
      context 'when compatibility is 2' do
        ...
        it 'does not raise an error if the compatibility is >= 2 and the provider is taskscheduler_api2' do
    ...
    describe '#flush' do
      ...
      describe 'when :ensure is :present' do
        ...
        it 'fails if the command is not specified' do
      ...
      describe 'when :ensure is :absent' do
        ...
        it 'does not save the task if :ensure is :absent' do
          mock_task.expects(:save).never

          resource.provider.flush
        end

        it 'does not fail if the command is not specified' do
    ...
    describe 'property setter methods' do
      ...
      describe '#command=' do
        it 'sets the application_name on the task' do
      ...
      describe '#arguments=' do
        it 'sets the parameters on the task' do
      ...
      describe '#working_dir=' do
        it 'sets the working_directory on the task' do
      ...
      describe '#enabled=' do
        ...
        it 'clears the enabled property if the task should be enabled' do
          mock_task.stubs(:enabled).returns(false)
      ...
      describe '#trigger=' do
        ...
        context 'with trigger' do
          let(:trigger) { { 'schedule' => 'once', 'start_date' => '2011-09-15', 'start_time' => '15:10' } }

          it 'does not consider all duplicate current triggers in sync with a single desired trigger' do
            current_triggers = [
          ...
          it 'removes triggers not defined in the resource' do
            current_triggers = [
              { 'schedule' => 'once', 'start_date' => '2011-9-15', 'start_time' => '15:10', 'index' => 0 },
        ...
        context 'with multiple triggers' do
          ...
          it 'adds triggers defined in the resource, but not found on the system' do
            current_triggers = [
              { 'schedule' => 'once', 'start_date' => '2011-9-15', 'start_time' => '15:10', 'index' => 0 },
            ]
            resource.provider.stubs(:trigger).returns(current_triggers)
      ...
      describe '#user=' do
        ...
        it 'uses nil for user and password when setting the user to the SYSTEM account' do
        ...
        it 'uses the specified user and password when setting the user to anything other than SYSTEM' do
