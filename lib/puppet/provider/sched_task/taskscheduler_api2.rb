...
Puppet::Type.type(:sched_task).provide(:taskscheduler_api2) do
  ...
  def validate_trigger(value)
    ...
    value.each do |t|
      if t.has_key?('index')
        self.fail "'index' is read-only on sched_task triggers and should be removed ('index' is usually provided in puppet resource sched_task)."
      end

      if t.has_key?('enabled')
        self.fail "'enabled' is read-only on sched_task triggers and should be removed ('enabled' is usually provided in puppet resource sched_task)."
      end
